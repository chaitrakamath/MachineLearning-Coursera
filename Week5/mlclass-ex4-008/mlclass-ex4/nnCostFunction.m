...
J_mat =((y_matrix .* log(h)) + ((1 - y_matrix) .* log(1 - h))) .* (-1/ m);
J = sum(sum(J_mat));
...
ThetaSum = sum((sum(Theta1_reg .^ 2))) + sum(sum(Theta2_reg .^2));
reg_exp = (ThetaSum * lambda) / (2 * m);
J = J + reg_exp;
...
Theta1_grad_reg = delta1 / m + (lambda * Theta1_reg) / m;
Theta2_grad_reg = delta2 / m + (lambda * Theta2_reg) / m;
